...
from video_processor.main import parse_args, apply_args_to_config, run_cli_mode
...
class TestCLIInterface:
    ...
    def test_parse_args(self):
        ...
        # Parse arguments
        with patch('sys.argv', ['video_processor'] + test_args):
            args = parse_args()
    ...
    def test_apply_args_to_config(self):
        ...
        class Args:
            def __init__(self):
                self.input = str(self.input_dir)
                self.output = str(self.output_dir)
    ...
    @patch('video_processor.processing.scheduler.ProcessingScheduler.process_videos')
    @patch('video_processor.processing.file_manager.FileManager.rename_files')
    @patch('video_processor.processing.file_manager.FileManager.organize_folders')
    def test_run_cli_mode(self, mock_organize, mock_rename, mock_process):
        """Test running in CLI mode"""
        # Mock component methods
        mock_rename.return_value = 2
        mock_process.return_value = True
        mock_organize.return_value = 2
    ...
    @patch('video_processor.main.main')
    def test_command_line_execution(self, mock_main):
        """Test executing the application from the command line"""
        # Mock the main function to return 0 (success)
        ...
        # Execute the module
        with patch('sys.argv', ['python', '-m', 'video_processor'] + test_args):
            # Import the module's __main__ to simulate command-line execution
            import video_processor.__main__
