...
class ParallelProcessingEfficiencyTest(PerformanceTest):
    ...
    def __init__(self, file_count: int, worker_count: int, iterations: int = 3):
        ...
        """
        super().__init__(
            f"Parallel Processing ({file_count} files, {worker_count} workers)",
            iterations,
        )
    ...
    def run_iteration(
        ...
        result, execution_time, memory_usage = time_and_memory_function(
...
class ProcessPoolScalingTest(PerformanceTest):
    ...
    def __init__(
        ...
        """
        super().__init__(
            f"ProcessPool Scaling ({task_count} tasks, {worker_count} workers)",
            iterations,
        )
    ...
    def run_iteration(self) -> PerformanceResult:
        ...
        def run_tasks():
            with ProcessPoolExecutor(max_workers=self.worker_count) as executor:
                futures = [
                    executor.submit(simulated_task, i) for i in range(self.task_count)
                ]
...
def test_parallel_processing_efficiency(mock_executor_class, mock_process_video_task):
    ...
    for worker_count in worker_counts:
        ...
        print(
            f"{worker_count:<10} {time_taken:<10.4f} {speedup:<10.4f} {efficiency:<10.4f}"
        )
