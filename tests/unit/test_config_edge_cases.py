...
class TestConfigEdgeCases:
    ...
    def test_load_incomplete_config(self):
        ...
        assert config.ffmpeg_params["encoder"] == incomplete_config["ffmpeg_params"]["encoder"]
    ...
    def test_validate_with_invalid_paths(self):
        ...
        # Set invalid paths
        config.input_folder = Path("/nonexistent/input")
        config.output_folder = Path("/nonexistent/output")
        
        # Validate
        errors, warnings = config.validate()
        
        # Check that validation detected the errors
        assert len(errors) >= 2
        assert any("input folder does not exist" in error.lower() for error in errors)
        assert any("output folder does not exist" in error.lower() for error in errors)
    ...
    def test_validate_with_invalid_ffmpeg_params(self):
        ...
        config.ffmpeg_params["encoder"] = "invalid_encoder"
