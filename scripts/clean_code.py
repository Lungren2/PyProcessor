...
def comment_unused_variables(check_only=False):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "vulture", "pyprocessor", "tests", "scripts", "--min-confidence", "90"],
        capture_output=True,
        text=True
    )
    ...
    for file_path, line_num, var_name in unused_vars:
        try:
            ...
            if 0 <= line_num - 1 < len(lines):
                ...
                if re.search(r'^\s*def\s+.*?\(.*?' + re.escape(var_name) + r'.*?\):', line):
                    continue
...
def main():
    parser = argparse.ArgumentParser(description="Clean code by removing unused imports and commenting unused variables")
    parser.add_argument("--imports-only", action="store_true", help="Only remove unused imports")
    parser.add_argument("--variables-only", action="store_true", help="Only comment unused variables")
    parser.add_argument("--check", action="store_true", help="Check for unused imports and variables without making changes")
