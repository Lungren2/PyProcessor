...
from pyprocessor.utils.process.gpu_manager import (
    get_gpus, has_encoding_capability, GPUCapability, get_all_gpu_usage
)
from pyprocessor.utils.file_system.temp_file_manager import (
    create_temp_dir, cleanup_temp_file, mark_temp_file_in_use, get_disk_space_info
)
...
class FFmpegEncoder:
    ...
    def build_command(self, input_file, output_folder):
        ...
        using_gpu = self.config.ffmpeg_params.get("video_encoder", "").endswith("_nvenc")
        if using_gpu:
            ...
            if gpu_usage and (gpu_usage.utilization > 0.8 or
                             (gpu_usage.encoder_usage and gpu_usage.encoder_usage > 0.8)):
                self.logger.warning(f"GPU is heavily utilized: {gpu_usage.utilization:.2%} util, "
                                  f"{gpu_usage.encoder_usage:.2%} encoder usage")
                self.logger.warning("Consider reducing batch size or using CPU encoding instead")

                # If memory is also constrained, switch to CPU encoding
                if gpu_usage.memory_utilization > 0.9:
                    self.logger.warning(f"GPU memory is critically low: {gpu_usage.memory_utilization:.2%} used")
                    self.logger.warning("Switching to CPU encoding due to GPU memory constraints")
        ...
        if using_gpu:
            ...
            if gpu_capabilities.get('gpu_count', 0) > 1:
                ...
                gpu_usages = get_all_gpu_usage()
                if gpu_usages:
                    best_gpu = max(gpu_usages, key=lambda x: (x.memory_total - x.memory_used))
                    cmd.extend(["-hwaccel", "cuda", "-hwaccel_device", str(best_gpu.index)])
    ...
    def encode_video(self, input_file, output_folder, progress_callback=None, encrypt_output=False, encryption_key_id=None):
        ...
        try:
            ...
            if disk_info.get('state') == 'critical':
                self.logger.error(f"Critical disk space: {disk_info.get('utilization', 0):.2%} used. Cannot proceed with encoding.")
                return False
            elif disk_info.get('state') == 'warning':
                self.logger.warning(f"Low disk space: {disk_info.get('utilization', 0):.2%} used. Proceed with caution.")
            ...
            if using_gpu:
                ...
                gpu_usage = self.get_gpu_usage()
                if gpu_usage:
                    self.logger.info(f"Initial GPU state: {gpu_usage.utilization:.2%} util, "
            ...
            if using_gpu:
                def gpu_monitor_callback(progress):
                    ...
                    if int(progress * 10) > int(self.encoding_progress * 10):
                        gpu_usage = self.get_gpu_usage()
                        if gpu_usage:
                            self.logger.info(f"GPU at {progress:.1%}: {gpu_usage.utilization:.2%} util, "
                        ...
                        if disk_info.get('state') == 'critical':
                            self.logger.error(f"Critical disk space during encoding: {disk_info.get('utilization', 0):.2%} used.")
            ...
            if using_gpu:
                gpu_usage = self.get_gpu_usage()
                if gpu_usage:
                    self.logger.info(f"Final GPU state: {gpu_usage.utilization:.2%} util, "
                                   f"{gpu_usage.memory_utilization:.2%} memory used, "
                                   f"{gpu_usage.encoder_usage:.2%} encoder usage")
            ...
            self.logger.info(f"Moving encoded files from temporary directory to output folder")

            # Check disk space in output folder before moving files
            disk_info = get_disk_space_info(output_folder)
            if disk_info.get('state') == 'critical':
                self.logger.error(f"Critical disk space in output folder: {disk_info.get('utilization', 0):.2%} used. Cannot move files.")
                return False
            ...
            if encrypt_output:
                ...
                encryption_success = self.encrypt_output(output_folder, encryption_key_id)
                if not encryption_success:
                    self.logger.warning(f"Encryption of output files in {output_folder} was not fully successful")
    ...
    def encrypt_output(self, output_folder, key_id=None):
        ...
        try:
            ...
            for file_path in all_files:
                # Encrypt the file
                success, encrypted_path, file_metadata = self.encryption_manager.encrypt_file(
                ...
                if success:
                    ...
                    encryption_metadata['files'][rel_path] = {
                        'encrypted_path': str(encrypted_path.relative_to(output_folder)),
            ...
            self.logger.info(f"Encrypted {success_count} of {len(all_files)} files in {output_folder}")
    ...
    def decrypt_input(self, input_folder):
        ...
        try:
            ...
            self.logger.info(f"Decrypted {success_count} of {total_files} files in {input_folder}")
