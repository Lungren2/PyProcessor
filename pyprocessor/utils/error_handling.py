...
class ErrorAdvice:
    ...
    def get_advice(cls, error: PyProcessorError) -> str:
        ...
        error_type = (
            error.details.get("error_type", "default") if error.details else "default"
        )
...
class ErrorRecovery:
    ...
    def recover_permission_denied(error: PyProcessorError) -> bool:
        ...
        if error.details and "path" in error.details:
            ...
            logger = get_logger()
            logger.warning(
                f"Permission denied for {path}. Please check file permissions."
            )
    ...
    def recover_disk_full(error: PyProcessorError) -> bool:
        ...
        if error.details:
            logger.debug(
                f"Attempting to recover from disk full error with details: {error.details}"
            )

        temp_dir = os.path.join(
            os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "ffmpeg_temp"
        )
    ...
    def recover_network_timeout(error: PyProcessorError) -> bool:
        ...
        if error.details:
            ...
            logger.warning(
                f"Network timeout occurred for {host}:{port}. This might be temporary, please try again later."
            )
        else:
            logger.warning(
                "Network timeout occurred. This might be temporary, please try again later."
            )
    ...
    def recover_encoding_error(error: PyProcessorError) -> bool:
        ...
        if error.details and "message" in error.details:
            ...
            if "No such file or directory" in message and "ffmpeg" in message.lower():
                ...
                logger.warning(
                    "Invalid input file. The file might be corrupted or in an unsupported format."
                )
...
def register_recovery_handlers():
    ...
    error_manager.register_recovery_handler(
        ErrorCategory.FILE_SYSTEM, ErrorRecovery.recover_file_not_found
    )
    error_manager.register_recovery_handler(
        ErrorCategory.PERMISSION, ErrorRecovery.recover_permission_denied
    )
    error_manager.register_recovery_handler(
        ErrorCategory.RESOURCE, ErrorRecovery.recover_disk_full
    )
    error_manager.register_recovery_handler(
        ErrorCategory.NETWORK, ErrorRecovery.recover_network_timeout
    )
    error_manager.register_recovery_handler(
        ErrorCategory.ENCODING, ErrorRecovery.recover_encoding_error
    )
                ...
                try:
                    ...
                    if fallback:
                        try:
                            ...
                            logger.error(
                                f"Fallback for {operation} also failed: {str(fallback_error)}"
                            )
...
def fallback_video_encoding(input_file, output_file, params):
    ...
    logger = get_logger()
    logger.info(
        f"Using simplified encoding parameters as fallback instead of: {params}"
    )

    # Use simpler encoding parameters
    simplified_params = {"codec": "libx264", "preset": "medium", "crf": "23"}
        ...
        def decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    cls.set_context(function=func.__name__, module=func.__module__)
...
def with_context_logging(func):
    ...
    def wrapper(*args, **kwargs):
        # Add function information to context
        ContextLogger.set_context(function=func.__name__, module=func.__module__)
...
class ErrorAggregator:
    ...
    def _generate_fingerprint(cls, error: PyProcessorError):
        ...
        components = [str(error.category), str(error)]
    ...
    def generate_report(cls, min_count=1, sort_by="count", limit=None):
        ...
        with cls._lock:
            ...
            fingerprints = [
                fp for fp, count in cls._error_counts.items() if count >= min_count
            ]
...
class UserFriendlyErrors:
    ...
    def get_cli_message(cls, error: PyProcessorError) -> str:
        ...
        template = cls._cli_messages.get(
            error.category, cls._cli_messages[ErrorCategory.UNKNOWN]
        )
    ...
    def get_api_error(cls, error: PyProcessorError) -> Dict[str, Any]:
        ...
        api_error = cls._api_errors.get(
            error.category, cls._api_errors[ErrorCategory.UNKNOWN]
        )
...
__all__ = [
    ...
    "clear_error_aggregation",
    # Retry mechanisms
    "RetryConfig",
    "with_retry",
    "retry_on_network_errors",
    "retry_on_resource_errors",
