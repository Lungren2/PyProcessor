...
class PasswordPolicy:
    ...
    def initialize(self, config=None):
        ...
        if config:
            if hasattr(config, "get"):
                ...
                self.min_length = config.get("security.password_policy.min_length", self.min_length)
                self.max_length = config.get("security.password_policy.max_length", self.max_length)
                self.require_uppercase = config.get("security.password_policy.require_uppercase", self.require_uppercase)
                self.require_lowercase = config.get("security.password_policy.require_lowercase", self.require_lowercase)
                self.require_digits = config.get("security.password_policy.require_digits", self.require_digits)
                self.require_special = config.get("security.password_policy.require_special", self.require_special)
                self.disallow_common = config.get("security.password_policy.disallow_common", self.disallow_common)
                self.disallow_username = config.get("security.password_policy.disallow_username", self.disallow_username)
                self.disallow_sequential = config.get("security.password_policy.disallow_sequential", self.disallow_sequential)
                self.disallow_repeated = config.get("security.password_policy.disallow_repeated", self.disallow_repeated)
                self.max_repeated_chars = config.get("security.password_policy.max_repeated_chars", self.max_repeated_chars)
    ...
    def validate_password(self, password: str, username: str = None) -> Tuple[bool, Optional[str]]:
        """
        ...
        if self.disallow_repeated:
            for i in range(len(password) - (self.max_repeated_chars - 1)):
                if all(password[i] == password[i+j] for j in range(1, self.max_repeated_chars)):
                    return False, f"Password must not contain more than {self.max_repeated_chars - 1} repeated characters"
...
def validate_password(password: str, username: str = None) -> Tuple[bool, Optional[str]]:
    """
    Validate a password against the password policy.
